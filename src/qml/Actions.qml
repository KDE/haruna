...
Item {
	...
	// Returns the gapless-safe hwdec equivalent for a given hwdec string.
	// Non-copy modes flush the GPU surface pool at the loop-file seek boundary;
	// copy modes keep decoded frames in system RAM and loop without a glitch.
	function gaplessHwdec(hwdec) {
		const copyMap = {
			"auto":         "auto-copy",
			"nvdec":        "nvdec-copy",
			"cuda":         "cuda-copy",
	...
	Connections {
		...
		function onToggleLoopFileAction() {
			const loopFile = root.m_mpv.getProperty(MpvProperties.LoopFile) === "inf"
			const newValue = loopFile ? "no" : "inf"
			PlaybackSettings.loopFile = !loopFile
			PlaybackSettings.save()
			root.m_mpv.setProperty(MpvProperties.LoopFile, newValue)
			// Coerce hwdec when enabling loop: non-copy modes stutter at the
			// loop-file seek boundary; copy modes stay in RAM and loop gaplessly.
			const configuredHwdec = PlaybackSettings.hWDecoding
			root.m_mpv.setProperty(MpvProperties.HardwareDecoding,
								   loopFile ? configuredHwdec : root.gaplessHwdec(configuredHwdec))
			root.m_osd.message(i18nc("@info:tooltip", "File Loop: %1", loopFile ? "Off" : "On"))
		}
