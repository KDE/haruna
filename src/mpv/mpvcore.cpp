...
int MpvCore::setProperty(const QString &name, const QVariant &value)
{
    mpv_node node;
    setNode(&node, value);
    int result = mpv_set_property(m_mpv, name.toUtf8().constData(), MPV_FORMAT_NODE, &node);
    if (result < 0) {
        qDebug() << name << errorString(result);
    }
    return result;
}
...
QString MpvCore::errorString(int err)
{
    switch (err) {
