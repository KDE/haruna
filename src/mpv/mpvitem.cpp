...
// Non-copy hwdec modes flush their GPU surface pool at the demuxer seek
// boundary that loop-file=inf inserts, producing a brief black frame.
// The corresponding -copy variants decode on the GPU but keep frames in
// system RAM, letting mpv loop seamlessly without reinitialising the decoder.
static QString toGaplessHwdec(const QString &hwdec)
{
	static const QHash<QString, QString> copyMap{
		{u"auto"_s,         u"auto-copy"_s},
		{u"nvdec"_s,        u"nvdec-copy"_s},
		{u"cuda"_s,         u"cuda-copy"_s},
		{u"d3d11va"_s,      u"d3d11va-copy"_s},
		{u"vaapi"_s,        u"vaapi-copy"_s},
		{u"vdpau"_s,        u"vdpau-copy"_s},
		{u"videotoolbox"_s, u"videotoolbox-copy"_s},
	};
	return copyMap.value(hwdec, hwdec);
}
...
void MpvItem::initProperties()
{
	...
	const QString configuredHwdec = PlaybackSettings::hWDecoding();
	const QString effectiveHwdec = PlaybackSettings::loopFile()
		? toGaplessHwdec(configuredHwdec)
		: configuredHwdec;
	Q_EMIT setProperty(MpvProperties::self()->HardwareDecoding, effectiveHwdec);
	Q_EMIT setProperty(MpvProperties::self()->LoopFile, PlaybackSettings::loopFile() ? u"inf"_s : u"no"_s);
